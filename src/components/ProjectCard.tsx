...
export function ProjectCard({ index, title, description, stack, image, url, delay = 0 }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="grid grid-cols-1 items-center gap-10 rounded-lg border border-border bg-card p-6 transition-colors duration-300 hover:border-primary/40 md:p-8 lg:grid-cols-2 lg:gap-14"
    >
      <div>
        <span className="font-mono text-xs text-muted-foreground">{index}</span>
        <h3 className="mt-4 text-2xl font-semibold tracking-[-0.015em] text-foreground">{title}</h3>
        <p className="mt-4 text-[15px] leading-[1.7] text-muted-foreground">{description}</p>
      ...
      <div className="overflow-hidden rounded-md border border-border bg-muted">
        ...
        <div className="aspect-[4/3] w-full">
          <img src={image} alt={`${title} website preview`} loading="lazy" className="h-full w-full object-cover object-top" />
        </div>
      </div>
    </motion.div>
  )
