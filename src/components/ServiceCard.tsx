...
export function ServiceCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col rounded-lg border border-border bg-card p-8 transition-all duration-300 hover:-translate-y-1 hover:border-primary/50 hover:bg-card-elevated hover:shadow-lg hover:shadow-primary/5",
    ...
    >
      <div className={cn("flex items-start justify-between", wide && "lg:w-64 lg:shrink-0 lg:flex-col lg:gap-6")}>
        ...
        <div className="flex h-11 w-11 items-center justify-center rounded-md border border-border bg-muted text-primary transition-shadow duration-300 group-hover:shadow-[0_0_16px_hsl(222_84%_48%/0.3)]">
          <Icon className="h-5 w-5" strokeWidth={1.5} />
        </div>
      </div>

      <div className={cn("flex flex-1 flex-col", wide && "lg:flex-1")}>
        <h3 className="mt-6 text-2xl font-bold tracking-[-0.015em] text-foreground lg:mt-0">{title}</h3>
